...
const TaskList: React.FC<TaskListProps> = ({ tasks, categories, onToggleTask, onDeleteTask, onEditTask }) => {
  if (tasks.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
        ...
        <p className="font-medium">No tasks found. Time to relax or create one!</p>
      </div>
    );
  }
          ...
          >
            ...
            <div className="flex-1 min-w-0" onClick={() => onEditTask(task)}>
              <div className="flex items-center gap-2 mb-1">
                <h3 className={`font-semibold text-slate-800 truncate ${task.completed ? 'line-through' : ''}`}>
                  {task.title}
                </h3>
                {cat && (
                  <span 
