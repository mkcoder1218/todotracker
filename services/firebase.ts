...
export const isFirebaseConfigured = firebaseConfig.apiKey !== "AIzaSyDummyKey_ReplaceWithActual" && !forceMock;
...
const getMockData = () => JSON.parse(localStorage.getItem(MOCK_STORAGE_KEY) || '{"tasks": [], "categories": []}');
...
export const collection = (db: any, path: string) => isFirebaseConfigured ? _collection(db, path) : path;
export const doc = (db: any, path: string, id: string) => isFirebaseConfigured ? _doc(db, path, id) : { path, id };
export const query = (ref: any, ...constraints: any[]) => isFirebaseConfigured ? _query(ref, ...constraints) : ref;
export const where = (field: string, op: string, val: any) => isFirebaseConfigured ? _where(field, op as any, val) : { field, op, val };
export const orderBy = (field: string, dir: string) => isFirebaseConfigured ? _orderBy(field, dir as any) : { field, dir };
...
export const updateDoc = async (docRef: any, data: any) => {
  ...
  const store = getMockData();
  const index = store[docRef.path].findIndex((item: any) => item.id === docRef.id);
  if (index !== -1) {
...
export const deleteDoc = async (docRef: any) => {
  ...
  const store = getMockData();
  store[docRef.path] = store[docRef.path].filter((item: any) => item.id !== docRef.id);
  saveMockData(store);
};

export const onSnapshot = (q: any, callback: (snapshot: any) => void) => {
  if (isFirebaseConfigured) return _onSnapshot(q, callback);
  const handler = () => {
    const store = getMockData();
    const data = store[q] || [];
    callback({
