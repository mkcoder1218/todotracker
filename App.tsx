...
const App: React.FC = () => {
  ...
  const handleSyncAlarms = () => {
    const tasksToSync = tasks.filter((t) => !t.completed && t.dueDate);

    if (tasksToSync.length === 0) {
      alert("No pending tasks with due dates to sync.");
      return;
    }

    // Find the next upcoming task
    const nextTask = tasksToSync.sort(
    ...
    if (!isAndroid) {
      alert(
        "Feature Unavailable on Desktop:\n\nSyncing to the native Alarm Clock uses Android Intents and only works on Android devices.\n\nPlease try this button on your mobile device!",
      );
      return;
    }
    ...
    if (
      nextTask &&
      confirm(
        `Syncing to native alarms is limited on the web.\n\nWould you like to set an alarm for the next upcoming task:\n"${nextTask.title}" at ${new Date(nextTask.dueDate).toLocaleString()}?`,
      )
    ) {
      const date = new Date(nextTask.dueDate);
      const hour = date.getHours();
      const minutes = date.getMinutes();
      // Construct Android Intent for setting an alarm
      const intentUrl = `intent:#Intent;action=android.intent.action.SET_ALARM;i.hour=${hour};i.minutes=${minutes};S.message=${encodeURIComponent(nextTask.title)};end`;

      // Try to open the intent (Works on Android)
          ...
          {currentView === "tasks" && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title="Sync deadlines to Device Alarm App"
